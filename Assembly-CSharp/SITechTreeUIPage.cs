...
public class SITechTreeUIPage : MonoBehaviour
{
	public void Configure(SITechTreeStation techTreeStation, SITechTreePage treePage, Transform imageTarget, Transform textTarget)
	{
		...
		float num = this.nodeContainer.rect.width / (float)count;
		for (int i = 0; i < count; i++)
		{
			float num2 = ((count < 2) ? 0f : (-22f + -num * (float)(count - 1) / 2f + num * (float)i));
			this.<Configure>g__AddNodes|5_0(null, treePage.Roots[i], vector + new Vector3(num2, 0f, 0f), ref CS$<>8__locals1);
		}
	...
	private void <Configure>g__AddNodes|5_0(GraphNode<SITechTreeNode> parent, GraphNode<SITechTreeNode> node, Vector3 position, ref SITechTreeUIPage.<>c__DisplayClass5_0 A_4)
	{
		...
		{
			num3 += graphNode.GetSubtreeWidth(int.MaxValue) - 1;
		}
		float num4 = (float)(50 + num3 * 25);
		...
		if (sitechTreeUINode.IsConfigured)
		{
			if (sitechTreeUINode.Parents.Count > 1)
			{
				...
				{
					num5 += sitechTreeUINode2.transform.localPosition.x;
				}
				position.x = num5 / (float)sitechTreeUINode.Parents.Count;
			}
		...
		for (int i = 0; i < count; i++)
		{
			float num6 = num + (float)((i + 1) % 2 * num2);
			GraphNode<SITechTreeNode> graphNode2 = node.Children[i];
			Vector3 vector = position + new Vector3(-num4 * (float)(count - 1) / 2f + num4 * (float)i, num6, 0f);
